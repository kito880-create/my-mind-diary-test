...
def get_prioritized_models(api_key):
    # API가 제공하는 실제 사용 가능한 모델 목록을 가져와 우선순위화
    url = f"https://generativelanguage.googleapis.com/v1beta/models?key={api_key}"
    try:
        resp = requests.get(url, timeout=10)
        if resp.status_code != 200: return ["gemini-1.5-flash", "gemini-1.5-flash-latest", "gemini-pro"]
        models = resp.json().get('models', [])
        candidates = [m.get('name', '').replace('models/', '') for m in models if 'generateContent' in m.get('supportedGenerationMethods', [])]
        # flash 모델을 최우선으로, 그 다음 pro 모델 순으로 정렬
        return sorted(candidates, key=lambda x: ('flash' not in x.lower(), 'pro' not in x.lower()))
    except: return ["gemini-1.5-flash", "gemini-1.5-flash-latest", "gemini-pro"]
    ...
    for model in model_list:
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                # 404(모델 없음)일 경우 다음 모델 시도
                continue
            else:
                st.error(f"API 요청 실패 (모델: {model}, Status: {resp.status_code}): {resp.text}")
                continue
        except Exception as e:
            st.warning(f"모델 {model} 시도 중 오류: {e}")
            continue
    return None
...
def generate_monthly_insight(api_key, text, t_v, s_v, f_v):
    ...
    return "분석 실패 (사용 가능한 AI 모델을 찾을 수 없습니다)."
