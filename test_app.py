...
def get_prioritized_models(api_key):
    # 한번 찾은 모델 목록은 세션 동안 저장(캐싱)하여 429 에러 방지
    if "cached_models" in st.session_state: return st.session_state["cached_models"]
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if candidates:
                res = sorted(candidates, key=lambda x: ('1.5-flash' not in x.lower(), 'flash' not in x.lower(), 'pro' not in x.lower()))
                st.session_state["cached_models"] = res
                return res
    except: pass
...
def analyze_diary(api_key, diary_text, tone_v, style_v, focus_v):
    ...
    prompt = f"""당신은 몽글곰팅 님의 심리 코치입니다. 말투:{t_ins}, 성향:{s_ins}, 방향:{f_ins}.
    [JSON 형식 필수] {{
        "mindfulness_board": [
        ...
        "cbt_analysis": {{
            "part1_main_emotions": ["주요감정"], "part1_sub_emotions": ["기타감정"], "part1_intensity": 0,
    ...
    for model in model_list[:3]: # 상위 3개까지 시도
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                st.error("⚠️ [429 Error] 사용 한도(Rate Limit)를 초과했습니다. 다시 시도 전 약 1~2분 정도 기다려 주세요.")
                return None
            elif resp.status_code == 404:
                continue # 다음 모델 시도
            else:
                st.error(f"❌ 분석 실패 (모델: {model}, Status: {resp.status_code})\n메시지: {resp.text[:200]}")
                return None
        except Exception as e:
            st.error(f"❌ 오류 발생: {str(e)[:50]}")
    return None
...
with t1:
    ...
    with col_date1:
        picked_date = st.date_input("날짜 이동", value=datetime.now())
        if st.button("해당 날짜로 이동"): 
            st.session_state["sel_date"] = picked_date.strftime("%Y-%m-%d")
            st.rerun()
    ...
    with col_date2:
        ...
        sel_date = st.select_slider(
            "최근 기록 바로가기",
            options=last_7_days,
            value=st.session_state["sel_date"] if st.session_state["sel_date"] in last_7_days else last_7_days[-1]
        )
        if sel_date != st.session_state["sel_date"]:
