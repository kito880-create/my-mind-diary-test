...
def get_prioritized_models(api_key):
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            if candidates:
                # 1.5-flash를 최우선으로(안정적), 그 다음 다른 flash, 그 다음 pro 순 정렬
                return sorted(candidates, key=lambda x: (
                    '1.5-flash' not in x.lower(), 
    ...
    for model in model_list[:4]: 
        ...
        try:
            ...
            if resp.status_code == 200:
                ...
                st.error("⚠️ [429 Error] 사용 한도(Rate Limit)를 초과했습니다. 무료 티어는 분당 요청 제한이 있으니 약 1~2분 후 다시 시도해 주세요.")
                return None
            errors.append(f"{model}: {resp.status_code}")
        except Exception as e:
            errors.append(f"{model}: {str(e)[:40]}")
    
    st.error(f"❌ 분석 실패. (오류: {', '.join(errors[:2])})")
    return None
